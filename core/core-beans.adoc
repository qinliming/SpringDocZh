...
=== 初始化一个容器
传输给 `ApplicationContext` 的构造方法的定位路径信息是一个资源字符串，这使得容器可以通过多种外部资源加载配置元数据，例如通过本地文件或者java CLASSPATH 等等。
...
[NOTE]
====
在你学习了Spring IoC容器之后你可以想了解更多关于Spring的 `Resources` 抽象概念(如 <<resources>>所述), 
它为从本地通过URI读取输入流提供了方便的机制，尤其， `Resource` 路径可以被用于初始化应用程序上下文，如<<resources-app-ctx>>所述。
====

以下代码展示了服务层定义的配置文件 `(services.xml)`
...
在前面的例子中，Service层由 `PetStoreServiceImpl` 和 `JpaAccountDao` `JpaItemDao` 两个DAO（基于JPA ORM标准）组成
`name` 属性 引用自javabean的属性, `ref` 引用了另一个bean的定义，通过 `id` 和 `ref` 这种连接描述了协作类之间的依赖关系，想了解更多关于对象依赖的内容 参考 
<<beans-dependencies,Dependencies>>.
...
bean可以定义在多个XML里面是非常有用的。通常，每一个独立的XML配置文件代表工程中的一个逻辑层或者模块。
你可以用applicationcontext 的构造器加载所有这些xml文件的beans定义如
<<beans-factory-instantiation, previous section>> 写的一样这个构造器可以加载多个资源.  
使用一个或者多个`<import/>` 去从一个bean配置文件里加载另一个。
以下配置文件展示了这个过程:
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<beans>
...
----
以上例子中，外部的bean定义加载自 `services.xml`, `messageSource.xml`, 和 `themeSource.xml` 三个文件。
所有这些路径都是相对于加载这些文件的xml的相对路径。所以 `services.xml` 必须和加载该文件的文件位于统一目录,同时
`messageSource.xml` 和 `themeSource.xml` 必须位于 在加载文件的文件目录下的 `resources` 中。
正如你所见到的，地一个/被忽略了，然而因为都用相对路径，所以最好不要在前面加/
文件的内容会被导入，包括开头的 `<beans/>` 它们必须是遵守Spring Schema的定义。

[NOTE]
====
可以通过../加载父目录的文件，但是我们不必推荐这么做。
这么做会让项目依赖项目外面的目录。尤其，在使用`classpath` URL 的时候不应该这样用例如（`classpath:../services.xml` ），因为运行时会选择“最近”的classpath的根目录然后搜索其父目录。然后当classpath配置改变的时候可能会引起路径错误等问题。 
你也可以用完整的路径而不用相对路径例如`file:C:/config/services.xml` 或者 `classpath:/config/services.xml`。
然而, 注意这样会使你的应用耦合于某个特定的目录。
更好的办法是，通过间接的方式指定绝对路径 -- 例如, 通过 "${...}" 这种可以被解析到JVM的环境变量的方式。

====

这个namespace本身提供了import这个指令的特性，在其他Spring提供的namepspace里有更多除了bean定义之外的特性，例如 `context` 和 `utils` namespace

[[groovy-bean-definition-dsl]]
==== Groovy Bean 定义 DSL

一个关于外部配置元数据定义的更加深入的例子，bean定义也可以通过Spring的groovy bean DSL进行描述，正如在grail框架里一样。
通常这些定义被写在 `.groovy` 文件里，如下文所述：
...
这种配置模式，基本和XML模式相同，甚至可以支持xml里面的namespace，它同样支持通过 `importBeans` 指令引入XML

[[beans-factory-client]]
=== 使用容器

`ApplicationContext` 是一个高级的能够管理beans和其依赖的注册表工厂的接口。 通过调用 `T getBean(String
name, Class<T> requiredType)`, 你可以检索你bean的实例。

`ApplicationContext` 使你能够读取bean定义并且获取他们, 如下文所示：
...
使用groovy配置，启动看起来很简单，它使用了不同的Groovy相关的上下文实现（同时它也能读取XMLbean配置）
下面的实例展示了这种配置模式
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
...
最有弹性可伸缩的方式是使用 `GenericApplicationContext` 并且配合读取标识 -- 例如，使用 `XmlBeanDefinitionReader` 读取XML 实例如下。
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
...
你也可以使用 `GroovyBeanDefinitionReader` 读取groovy配置 如下文所示:
...
你可以在同一个 `ApplicationContext` 上混用这些读取器标识，从不同的配置文件中获取bean定义。

然后，你就可以用 `getBean` 检索beans的实现，`ApplicationContext`也有几个其他的方法来检索beans，但是，明智的是不要去使用那些方法。
事实上你的应用也不应该去调用 `getBean()` 方法，并且因此不需要以来Spring的API。例如Spring集成到web框架中去为不同的web模块提供依赖注入例如，控制器和JSF管理的bean，允许你通过一个特殊的包含在bean里的元数据定义依赖。（例如，autowire注解）


[[beans-definition]]
== Bean 概述

Spring IoC容器管理一个或者多个bean，这些bean是通过你提供给容器的配置元数据而进行创建的。（例如，基于XML的配置文件里面的 `<bean/>`）


在container内部，这些定义是通过 `BeanDefinition` 来表示的，它包含一下元数据(包括一些其他信息)。

* 一个包相关的类名：一般是确切的bean实现类。
* Bean行为的配置元素，它定义了bean在容器内的行为。(scope, lifecycle callbacks等等).
* 引用其他的它需要的bean去完成工作，这种引用通常叫做协作者或者依赖。
* 其他的需要在新建立的对象上被设置的内容 -- 例如, 在连接池管理类里设置连接池的大小
这些元数据被翻译成组成bean 定义类的属性，下面这个表展示了这些属性。
